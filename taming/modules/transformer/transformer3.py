...
class Transformer3(nn.Module):
    ...
    def forward(self, x: Tensor):
        ...
        #     len(in_token), 1, 1
        # )
        src_token = in_token[:, 0, :].unsqueeze(1)
        src_token = src_token.permute(1, 0, 2)
        # print(in_token.shape, out_token.shape)
        # tgt_token = in_token[:, :-1, :].view(in_token.shape[0], -1, in_token.shape[-1])
        tgt_token = in_token.permute(1, 0, 2)
        
        # print(tgt_token.permute(1, 0, 2).shape, src_token.permute(1, 0, 2).shape)
        out = self.transformer(
            src=src_token, tgt=tgt_token
        )
