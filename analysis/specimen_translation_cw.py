...
def main(configs_yaml):
    ...
    model = load_CWVQGAN(config, ckpt_path=ckpt_path, cuda=(DEVICE is not None), model_type=CWmodelVQGAN)
    ...
    for indx__ in tqdm.tqdm(range(count)):
        ...
        fig_path = os.path.join(str(fig_path), "transitions_old", dataset.indx_to_label[classes[0]] + ' to ' + dataset.indx_to_label[classes[1]])
